...
def parse():
    ...
    parser.add_argument(
        "-c", "--corpus", help="Test the saved model with vocabulary of the corpus"
    )
    parser.add_argument(
        "-r", "--reverse", action="store_true", help="Reverse the input sequence"
    )
    parser.add_argument(
        "-f", "--filter", action="store_true", help="Filter to small training data set"
    )
    parser.add_argument(
        "-i",
        "--input",
        action="store_true",
        help="Test the model by input the sentence",
    )
    parser.add_argument(
        "-it",
        "--iteration",
        type=int,
        default=10000,
        help="Train the model with it iterations",
    )
    parser.add_argument(
        "-p", "--print", type=int, default=100, help="Print every p iterations"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Hidden size in encoder and decoder",
    )
    parser.add_argument(
        "-be", "--beam", type=int, default=1, help="Hidden size in encoder and decoder"
    )
    parser.add_argument(
        "-s", "--save", type=int, default=500, help="Save every s iterations"
    )
    parser.add_argument(
...
def run(args):
    reverse, fil, n_iteration, print_every, save_every, learning_rate, n_layers, hidden_size, batch_size, beam_size, inp, dropout = (
        args.reverse,
        args.filter,
        args.iteration,
        args.print,
