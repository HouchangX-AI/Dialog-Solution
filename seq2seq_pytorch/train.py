    ...
    if use_teacher_forcing:
        for t in range(max_target_len):
            ...
            loss += F.cross_entropy(
                decoder_output, target_variable[t], ignore_index=EOS_token
            )
    else:
        for t in range(max_target_len):
...
def trainIters(
    ...
    try:
        training_batches = torch.load(
            os.path.join(
                ...
                corpus_name,
                "{}_{}_{}.tar".format(
                    n_iteration, filename(reverse, "training_batches"), batch_size
                ),
            )
        )
        ...
        training_batches = [
            batch2TrainData(
                voc, [random.choice(pairs) for _ in range(batch_size)], reverse
            )
    ...
    decoder = LuongAttnDecoderRNN(
        attn_model, embedding, hidden_size, voc.n_words, n_layers, dropout
    )
    if loadFilename:
