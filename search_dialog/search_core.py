...
class SearchCore(object):
    def __init__(self):
        self.word2id, _ = loadDataset(vocab_size=10000)
        if SEARCH_MODEL == "tfidf":
            self.qa_search_inst = TfidfModel(
                search_dialog.config.question_answer_path,
                scale="large",
                word2id=self.word2id,
            )
            self.cr_search_inst = TfidfModel(
                search_dialog.config.context_response_path,
                scale="large",
                word2id=self.word2id,
            )
        elif SEARCH_MODEL == "bm25":
            self.qa_search_inst = BM25Model(
                search_dialog.config.question_answer_path, word2id=self.word2id
            )
            self.cr_search_inst = BM25Model(
                search_dialog.config.context_response_path, word2id=self.word2id
            )
    ...
    def search(self, msg_tokens, mode="qa", filter_pattern=None):
        ...
        log_print(
            "init_query=%s, filter_query=%s" % ("".join(msg_tokens), "".join(query))
        )
