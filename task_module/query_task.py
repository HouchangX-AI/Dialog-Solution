...
def query_judge(sentence, dialog_status):
    ...
    if True:
        ...
        if re.search("快递|订单", sentence):
            if re.search("哪儿", sentence):
                return "您好，登陆[链接x]输入运单号即可查询物流信息哦"
            if re.search("价格", sentence):
                return "价格为[数字x]元哦"
            if re.search("取消", sentence):
                if re.search("怎么|如何", sentence):
                    return "登陆京东app客户端，点击页面右下方【我的】，进入【我的订单】页面查看即可"
                return "您好，如果您需要取消，小妹这边可以帮您提交申请的哦"
            # return "您好，有什么我可以帮您处理或解决呢"

        if re.search("订单号|ORDERID_DIGIT|商品", sentence):
            if re.search("改", sentence):
                return "您好，请问您有什么需要修改的呢"
